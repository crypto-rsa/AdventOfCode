    ...
    private record Component(char Name, HashSet<GridPosition> Plots)
    {
        ...
        public int GetPerimeter()
        {
            ...
            int GetPerimeterEdgesToRemove(GridPosition plot)
                => _grid.GetNeighbours(plot, includeDiagonals: false).Count(neighbour => Plots.Contains(neighbour));
        }
    ...
    private static List<Component> GetComponents()
    {
        ...
        foreach (var position in grid)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                foreach (var neighbour in grid.GetNeighbours(current, includeDiagonals: false))
                {
